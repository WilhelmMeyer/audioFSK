...
- **Quando:** 2026-09-03 11:15:15  (campanha de 2.2 min)
...
Melhor caso medido: M-aria 16 tons, ganho 0.30 escolhido pela varredura anterior (0.20/0.25/0.30/0.38), FEC rate 1/3 repetida duas vezes, varredura de sincronismo nas duas pontas. Doze gravacoes. As colunas gate e varredura saem do MESMO audio, entao a comparacao nao tem sala nem momento diferentes dentro dela.

## Resultado

| condicao | n | blocos (gate) | blocos (varredura) | bits (gate) | bits (varredura) | periodo |
|---|---|---|---|---|---|---|
| unico | 12 | 11/12 | 12/12 | 89.0% | 90.4% | 480.00 |

Comparacao pareada, mesmo audio nas duas colunas: a varredura leu mais bits que o gate em 12 de 12 gravacoes.

Nivel recebido: rms 0.114, pico 0.55 (maior 0.62).
...
```bash
./venv/bin/python study.py --rescore studies/20260903-111515-melhor-caso-mary
```
