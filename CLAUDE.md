...
**Compare paired, over the same recording, not by two averages.** Both the gate and the sweep columns are read off the same audio, so "the sweeps read more bits than the gate on 24 of 24 recordings" is a statement with no room and no moment inside it. Across the three Bluetooth campaigns at the calibrated point it is **59 of 60** -- where the averages differ by one to two points over twelve noisy trials and would not, alone, establish an effect. Scope the claim to its corpus: on the sixty wired recordings the same paired count is 8 of 60, the other way.

**Block recovery is not a function of bit accuracy, and the clean channel is what proved it.** Two corpora of sixty self-capture recordings each, same layer and same settings: Bluetooth at the calibrated point (deliberately the harder path) and the internal wired speaker at `gain 0.20` (deliberately the easy one).

| corpus | bits, gate | bits, sweeps | blocks, gate | blocks, sweeps |
|---|---|---|---|---|
| Bluetooth, degraded on purpose | 88.8% | **90.8%** | 43/60 | **59/60** |
| wired, clean on purpose | **96.3%** | 93.8% | 26/60 | **57/60** |

The wired channel reads 7.5 points more bits and the gate recovers *fewer* whole blocks on it -- 26 of 60 against 43. Rate 1/3 is whole to about 13% of bits wrong, so 4% error should decode with room to spare, and it does not: what the gate loses is its own alignment, and the bit column cannot show that because bit accuracy is scored at the best brute-forced slide while block recovery is scored where the gate actually landed. The sweeps sit at 59/60 and 57/60 -- essentially the same on both channels, which is the property being bought.

**On a clean channel the sweeps read *fewer* bits than the gate and are still the right choice.** They won the bit comparison on 59 of 60 Bluetooth recordings and on 8 of 60 wired ones: a frozen clock cannot follow what a steering gate can, and on a channel with margin there is nothing to rescue. Blocks say the opposite, and blocks are what a link delivers. Half a point of bit accuracy is worth zero collapses, because no coding rate repairs a block whose symbols were never sampled where the symbols are.

**A gate collapse can persist for minutes and is not visible in the levels.** The wired path was measured three times at identical settings: six recordings gave the gate 6 of 6, then twelve gave 1 of 12, then twelve more gave 3 of 12 -- with received rms between 0.079 and 0.084 in all three and the sweeps at 6/6, 11/12 and 11/12 over the same audio. **The cause is not known.** It is not level, not clipping, and not sample-rate drift (this path shares the soundcard clock and the measured period is 480.00). Do not average across such a window and do not read six recordings as a result; the first reading here would have been reported as `6/6` and was contradicted twice.

**Say which corpus was recorded on a channel degraded on purpose, because two of them were.** A clean channel cannot show a synchronisation improvement -- everything decodes either way and the comparison measures nothing -- so the sweeps were developed against `gain 0.45` on the Bluetooth self-capture path, where three recordings of eight arrive at peak 1.00 and the limiter is flattening the signal. That is a deliberate impairment and a legitimate one, but it is *distortion*, not noise, and a number read off it does not transfer to a clean link. State the stressor beside the number or the number is unreadable. Once a mechanism is validated under degradation, re-measure at the operating point: swept properly (`study.py --sweep gain=0.20,0.25,0.30,0.38`, `fecrep 1`, six recordings each) **0.30** is the best point on that path and clips nowhere -- 90.1% of bits with the gate and 91.5% with the sweeps, against 88.1-88.2% at the neighbours. Calibrate at `fecrep 1`: at `fecrep 2` every condition recovers every block and the comparison measures nothing.
