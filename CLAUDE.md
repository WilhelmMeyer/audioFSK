...
**The measured channel, which supersedes anything assumed about it.** Band 550-3500 Hz is usable. Above 4 kHz SNR collapses (12 dB at 4000, 8 dB at 4500, ~0 at 5000) and above 6 kHz it is negative, with the level pinned at the noise floor — so **ultrasound is not viable on this hardware**; the argument for it is right (rooms are quiet up there) and the transducers simply do not reach. The response is a *comb*, not a curve: at 50 Hz resolution neighbouring bins differ by 13-18 dB, and two of the original ten MFSK tones sat in nulls and voted like coins. And there is **no measurable reverberation** — the tail after a burst does not decay, it stops at the noise floor. That is a statement about a burst in the working band, and it is not the only tail measured here: after the A->B sweep, in the 5600-6400 Hz band where the ramp ended and with a 512-sample window, the level falls about 50 dB over half a second and only reaches the room floor near 0.8 s (`resultados/03-CH-CHIRP-A2B/figuras/COMO-REFAZER.md`). The transmitter's own fade is 10 ms, so it is not that; one recording does not separate the room from the Bluetooth speaker and its codec, so the origin stays unattributed. Quote whichever measurement matches the question, and say which. Re-measure with `capture.py --chirp` plus `channel.py` before choosing any frequency.
