...
**O eco entra como propriedade do meio, nunca como dificuldade que enfrentamos.** O projeto antecipa reverberação, e é por isso que existe o intervalo de guarda, 15% de cada símbolo, 72 amostras de 480, descartadas antes de medir (`guard=0.15` nos dois demoduladores de `modem.py`; os 35% que este arquivo afirmava até 2026-09-07 são um caso de teste do `loopback_test.py`, ajustado para reverberação simulada, e nunca foram o ajuste do enlace). A seção 2.1 apresenta o fenômeno com gancho para os resultados, e eles fecham o gancho com o que mediram: depois da varredura o nível não cai de uma vez, e uma gravação só não separa a sala do alto-falante sem fio e do codec dele. A frase anterior desta doutrina, de que esta bancada não tem cauda mensurável, foi retirada em 2026-09-07 porque a seção 4 mediu o contrário; ela vinha do `CLAUDE.md` da raiz, que fala de burst na banda de trabalho e não da cauda da varredura em 6 kHz. Se o eco entrasse na introdução como obstáculo vencido, os resultados desmentiriam o texto. As dificuldades que a bancada de fato encontrou são o pente, a saturação da cadeia analógica, o viés de amplitude da 2-FSK, os relógios independentes e o alinhamento de nibble da 16-FSK.
...
- `simeca-md/`: o conversor, **versionado dentro deste repositório** (decisão do autor, 2026-09-06, revendo a de mantê-lo como submódulo). Quem clona o audioFSK já tem tudo para montar o artigo, sem passo de submódulo e sem depender de o clone ter sido feito de um jeito específico, que era o que atrapalhava o segundo autor no Windows. A cópia veio de `github.com/WilhelmMeyer/simeca-md`, sincronizada à mão em 2026-09-08 no commit `7dc36be` de lá; o preço é que ela não recebe correção de lá nem devolve correção para lá sozinha, e a sincronia com o artigo da roda de reação, que usa o mesmo conversor, é trabalho manual. Correção no conversor se faz aqui e, se valer para os dois artigos, se leva à mão para o outro lado — foi o que aconteceu com o conserto do Windows (`86bc38f` aqui), escrito neste repositório e só depois levado para lá. **As duas cópias divergiram nos dois sentidos antes de alguém olhar**, o que é o custo previsto desta decisão e não um acidente: compare pasta contra pasta antes de copiar por cima, porque a mais nova não é a certa em todos os arquivos.
